 ...
 * @copyright Copyright (c) 2013-2014 Quite Nice Stuff
...
[HOTEL_NAME]',
			'manually_confirm_bookings' => 0,
			'disable_database' => 0,
			'hide_booking_system' => 0,
			'enable_smtp' => 0,
			'smtp_host' => '',
			'smtp_port' => '',
			'smtp_encryption' => '',
...
[textarea id="special_requirements" label="Special Requirements"]',
			'enable_live_calendar_step_1' => 0,
			'enable_live_calendar_single' => 0,
			'enable_live_calendar_widget' => 0,
			'email_user_template' => 'Dear [guest_first_name] [guest_last_name],
...
[multi_room_booking]
<strong>Room [room_number] Info</strong>
<ul>
	<li><strong>Room Type:</strong> [room_type]</li>
	<li><strong>Date:</strong> [checkin] - [checkout]</li>
	<li><strong>Guests:</strong> [number_of_guests]</li>
...
'
		...
		$this->allowed_settings = array_keys($this->default_settings);
	}
	...
	public function admin_menu() {
		...
		if (class_exists('Quitenicebooking_Email_Templates')) {
			$html_email = new Quitenicebooking_Email_Templates($this->settings);
		}
		$reservation_form = new Quitenicebooking_Reservation_Form($this->settings);
		...
		$tab_includes = array(
			...
			'forms' => plugin_dir_path(__FILE__) . '../views/admin/admin_forms.htm.php'
		);
	...
	public function sanitize_settings($post) {
		...
		if ( isset( $post['disable_database'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['disable_database'] ) ) {
				$post['disable_database'] = $this->settings['disable_database'];
			}
		}

		// validate hide booking system
		if (isset($post['hide_booking_system'])) {
			if (!preg_match('/^[01]$/', $post['hide_booking_system'])) {
				$post['hide_booking_system'] = $this->settings['hide_booking_system'];
			}
		}
		...
		if ( isset( $post['entity_scheme' ] ) ) {
			...
			if ($this->settings['entity_scheme'] != $post['entity_scheme']) {
				$errors[] = __('Notice: Pricing scheme changed. Accommodations have been set to "pending" status to prevent pricing errors.  Please review or enter new prices for each accommodation and then publish them.', 'quitenicebooking');
				global $wpdb;
				$wpdb->update(
					"{$wpdb->posts}",
					array('post_status' => 'pending'),
					array('post_type' => 'accommodation', 'post_status' => 'publish')
				);
			}
		}
		...
		if ( isset( $post['remove_children'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['remove_children'] ) ) {
		...
		if ( isset( $post['multiroom_link'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['multiroom_link'] ) ) {
		...
		if (isset($post['minimum_stay'])) {
			if (!preg_match('/^\d+$/', $post['minimum_stay']) || ($post['minimum_stay'] < 1)) {
				$post['minimum_stay'] = $this->settings['minimum_stay'];
			}
		}
		...
		if (isset($post['currency_decimals'])) {
			if (!preg_match('/^\d+$/',$post['currency_decimals'])) {
				$post['currency_decimals'] = $this->settings['currency_decimals'];
				$errors[] = __('Please enter a valid number of currency decimal places', 'quitenicebooking');
			} else {
				$post['currency_decimals'] = intval($post['currency_decimals']);
			}
		}
		...
		// validate currency thousands/decimal separator
		if (isset($post['currency_thousands_separator']) || isset($post['currency_decimal_separator'])) {
			// trim separators down to a single character
			$post['currency_thousands_separator'] = substr($post['currency_thousands_separator'], 0, 1);
			$post['currency_decimal_separator'] = substr($post['currency_decimal_separator'], 0, 1);

			// if decimal is not net
			if (strlen($post['currency_decimal_separator']) == 0) {
				$post['currency_decimal_separator'] = $this->settings['currency_decimal_separator'];
				$errors[] = __('Please enter a currency decimal separator', 'quitenicebooking');
			}

			// if both separators are the same
			if ($post['currency_thousands_separator'] == $post['currency_decimal_separator']) {
				$post['currency_thousands_separator'] = $this->settings['currency_thousands_separator'];
				$post['currency_decimal_separator'] = $this->settings['currency_decimal_separator'];
				$errors[] = __('Currency thousands and decimal separators cannot be the same', 'quitenicebooking');;
			}

			// if either separator is a digit
			if (preg_match('/\d/', $post['currency_thousands_separator'])) {
				$post['currency_thousands_separator'] = $this->settings['currency_thousands_separator'];
				$errors[] = __('Thousands separator cannot be a number', 'quitenicebooking');;
			}
			if (preg_match('/\d/', $post['currency_decimal_separator'])) {
				$post['currency_decimal_separator'] = $this->settings['currency_decimal_separator'];
				$errors[] = __('Decimal separator cannot be a number', 'quitenicebooking');;
			}
		}
		...
		if ( isset($post['deposit_type'] ) ) {
			if ( ! in_array( $post['deposit_type'], array( '0', 'percentage', 'flat', 'duration', 'full' ) ) ) {
		...
		// validate tax rate
		if (isset($post['tax_percentage'])) {
			if (!preg_match( '/^(\.\d+|\d+\.?\d*)$/', $post['tax_percentage'])) {
				$post['tax_percentage'] = $this->settings['tax_percentage'];
				$errors[] = __( 'Please enter a valid tax rate', 'quitenicebooking' );
			}
		}

		// validate enable coupons
		if ( isset( $post['enable_coupons'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['enable_coupons'] ) ) {
				$post['enable_coupons'] = $this->settings['enable_coupons'];
			}
		}

		// validate paypal
		if ( isset( $post['accept_paypal'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['accept_paypal'] ) ) {
		...
		if ( isset( $post['accept_bank_transfer'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['accept_bank_transfer'] ) ) {
		...
		if ( isset( $post['uninstall_wipe'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['uninstall_wipe'] ) ) {
		...
		// validate enable live availability calendar
		if ( isset( $post['enable_live_calendar_step_1'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['enable_live_calendar_step_1'] ) ) {
				$post['enable_live_calendar_step_1'] = $this->settings['enable_live_calendar_step_1'];
			}
		}
		if ( isset( $post['enable_live_calendar_single'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['enable_live_calendar_single'] ) ) {
				$post['enable_live_calendar_single'] = $this->settings['enable_live_calendar_single'];
			}
		}
		if ( isset( $post['enable_live_calendar_widget'] ) ) {
			if ( ! preg_match('/^[01]$/', $post['enable_live_calendar_widget'] ) ) {
				$post['enable_live_calendar_widget'] = $this->settings['enable_live_calendar_widget'];
			}
		}

		$post = apply_filters('quitenicebooking_reservation_form_save', $post);
...
<?php
	...
	public function maintenance_reset(){
		...
		$status = update_option('quitenicebooking', $this->default_settings);
		// clears user meta
	...
	public function maintenance_repair_accommodations() {
		...
		if (is_object($accommodation_post_class->price_filters_class) && get_class($accommodation_post_class->price_filters_class) == 'Quitenicebooking_Price_Filters') {
	...
	public static function activate() {
		...
		if ($existing_settings === FALSE) {
			...
			if (get_option('qns_booking_booking1_url') !== FALSE) {
				...
				$settings['currency_unit_suffix'] = 0;
				$settings['currency_thousands_separator'] = ',';
				$settings['currency_decimal_separator'] = '.';
				$settings['currency_decimals'] = 0;
				$settings['deposit_type'] = get_option('qns_deposit_percentage') == '0' ? '0' : 'percentage';
				...
				$settings['accept_paypal'] = get_option('qns_paypal_deposit') == 'on' ? 1 : 0;
				...
				$settings['manually_confirm_bookings'] = 0;
				$settings['disable_database'] = get_option('qns_record_booking') == 'on' ? 1 : 0;
				$settings['hide_booking_system'] = 0;
				$settings['remove_children'] = get_option('qns_remove_children') == 'on' ? 1 : 0;
				$settings['multiroom_link'] = 0;
				$settings['rooms_order'] = get_option('qns_rooms_order');
				$settings['rooms_per_page'] = get_option('qns_rooms_per_page');
				$settings['accept_bank_transfer'] = 0;
...
[text required id="country" label="Country"]
[textarea id="special_requirements" label="Special Requirements"]';
				$settings['enable_live_calendar_step_1'] = 0;
				$settings['enable_live_calendar_single'] = 0;
				$settings['enable_live_calendar_widget'] = 0;
				...
				Quitenicebooking_Email_Templates::activate();
...
'
				...
				update_option('quitenicebooking', $default_settings);
...
[textarea id="special_requirements" label="Special Requirements"]';
		...
		/*
		if ($version >= 2.50 && $version < 2.53) {
			// add the default_label parameter to the form fields (non-premium)
			$settings = get_option('quitenicebooking');
			$settings['reservation_form'] = str_replace('"]', '" default_label]', $settings['reservation_form']);
			remove_all_filters('pre_update_option_quitenicebooking');
			update_option('quitenicebooking', $settings);
			$updated = TRUE;
		}
		*/
		...
		if ($version >= 2.00 && $version < 2.55) {
			// add disable_calendar_availability setting
			// add currency_decimals setting
			$settings = get_option('quitenicebooking');
			$settings['disable_calendar_availability'] = 0;
			$settings['currency_decimals'] = 0;
			remove_all_filters('pre_update_option_quitenicebooking');
			update_option('quitenicebooking', $settings);
			$updated = TRUE;
		...
		if ($version >= 2.00 && $version < 2.56) {
			// add minimum stay
			// add hide booking system
			// add enable coupons
			$settings = get_option('quitenicebooking');
			$settings['minimum_stay'] = 1;
			$settings['hide_booking_system'] = 0;
		...
		if ($version >= 2.53 && $version < 2.56) {
			// remove the default_label parameter from the form fields
			$settings = get_option('quitenicebooking');
			$settings['reservation_form'] = str_replace(' default_label', '', $settings['reservation_form']);
			remove_all_filters('pre_update_option_quitenicebooking');
			update_option('quitenicebooking', $settings);
		...
		if ($version >= 2.00 && $version < 2.58) {
			$settings = get_option('quitenicebooking');

			$settings['manually_confirm_bookings'] = 0;
			$settings['enable_live_calendar_step_1'] = empty($settings['disable_calendar_availability']) ? 1 : 0;
			$settings['enable_live_calendar_single'] = 0;
			$settings['enable_live_calendar_widget'] = 0;
			unset($settings['disable_calendar_availability']);
			$settings['tax_percentage'] = 0;

			// WPML checkbox fix
			if (empty($settings['disable_database'])) { $settings['disable_database'] = 0; }
			if (empty($settings['hide_booking_system'])) { $settings['hide_booking_system'] = 0; }
			if (empty($settings['remove_children'])) { $settings['remove_children'] = 0; }
			if (empty($settings['multiroom_link'])) { $settings['multiroom_link'] = 0; }
			if (empty($settings['currency_unit_suffix'])) { $settings['currency_unit_suffix'] = 0; }
			if (empty($settings['enable_coupons'])) { $settings['enable_coupons'] = 0; }
			if (empty($settings['accept_paypal'])) { $settings['accept_paypal'] = 0; }
			if (empty($settings['accept_bank_transfer'])) { $settings['accept_bank_transfer'] = 0; }
			if (empty($settings['enable_smtp'])) { $settings['enable_smtp'] = 0; }
			if (empty($settings['smtp_auth'])) { $settings['smtp_auth'] = 0; }
			if (empty($settings['uninstall_wipe'])) { $settings['uninstall_wipe'] = 0; }
			if (empty($settings['deposit_type'])) { $settings['deposit_type'] = 0; }
	...
	public static function install_pages() {
		...
		if ( isset($get['page']) && $get['page'] == 'quitenicebooking_settings' ) {
			...
			if ( isset($get['install_pages']) && $get['install_pages'] == '1' ) {
				...
				wp_redirect( admin_url( 'admin.php?page=quitenicebooking_settings&tab=pages&pages_installed=1' ) );
			}
		}
	}
...
PRIMARY KEY  (id)
	...
	/**
	 * Pages installed notice
	 */
	public static function pages_installed_notice() {
		?>
		<div class="updated">
		<p>QNS Booking pages have been successfully installed.</p>
		<p>
			<a class="button" id="qns_close_notice">Dismiss</a>
		</p>
		<script>
			jQuery('#qns_close_notice').on('click', function(e) { e.preventDefault(); jQuery(this).parent().parent().css('display', 'none'); });
		</script>
		</div>
		<?php
	}
	...
	/**
	 * MySQL privilege check notice
	 */
	public static function database_privilege_notice() {
		global $wpdb;
		ob_start(); // discard error messages
		$wpdb->query("CREATE TEMPORARY TABLE IF NOT EXISTS {$wpdb->prefix}temp (test varchar(255) NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8");
		$wpdb->insert("{$wpdb->prefix}temp", array('test' => 'one'));
		$r = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}temp", ARRAY_A);
		ob_get_clean(); // discard error messages
		if (!isset($r[0]['test']) || $r[0]['test'] != 'one') {
			?>
			<div class="error">
				<p>Please grant your MySQL user <a href="http://dev.mysql.com/doc/refman/5.5/en/privileges-provided.html" target="_blank">privileges</a> to create and update temporary tables.  Temporary tables are required for the QNS Booking plugin to function properly.  If you are unsure how to do this, please ask your web hosting provider for assistance.</p>
			</div>
			<?php
		}
	}
