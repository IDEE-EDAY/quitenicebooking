...
class Quitenicebooking_Accommodation_Post {
	...
	public function admin_enqueue_scripts() {
		...
		if (is_object($post) && get_post_type($post->ID) == 'accommodation') {
			...
			wp_localize_script('quitenicebooking-accommodation-admin', 'quitenicebooking', array(
				...
				'num_price_rules' => $this->get_num_price_rules($post->ID)
			));
			wp_enqueue_style('quitenicebooking-admin-shared', QUITENICEBOOKING_URL.'assets/css/admin/shared.css');
			wp_enqueue_style('quitenicebooking-accommodation-admin', QUITENICEBOOKING_URL.'assets/css/admin/accommodation_meta.css');
	...
	public function show_pricing_meta_box() {
		...
		$quitenicebooking_num_price_rules = $this->get_num_price_rules($post->ID);
	...
	public function save_meta($post_id) {
		...
		// prevent meta from being overwritten when not saving via the accommodation editor
		if (empty($_REQUEST['quitenicebooking_update_accommodation'])) {
			return;
		}
		...
		$price_class = 'Quitenicebooking_Prices_'.ucfirst($this->settings['pricing_scheme']);
		$quitenicebooking_prices = new $price_class($this->settings['entity_scheme']);
		$quitenicebooking_prices->settings = $this->settings;
		$quitenicebooking_prices->save_meta($post_id, filter_input_array(INPUT_POST));
		// update surcharge
		if (isset($_REQUEST['quitenicebooking_surcharge'])) {
			update_post_meta($post_id, 'quitenicebooking_surcharge', Quitenicebooking_Utilities::user_price_to_float($_REQUEST['quitenicebooking_surcharge'], $this->settings));
		}
		// update filters
		...
		if (is_a($quitenicebooking_prices->entity_scheme, 'Quitenicebooking_Entity_Per_Person')) {
			...
			foreach ($_REQUEST as $key => $value) {
				if (preg_match('/^quitenicebooking_price_rule_\d+/', $key)) {
					update_post_meta($post_id, $key, Quitenicebooking_Utilities::user_price_to_float($value, $this->settings));
				}
				// increment count
				if (preg_match('/^quitenicebooking_price_rule_\d+_adult_weekday/', $key)) {
					$num_price_rules ++;
				}
			}
			// delete all price rules above $num_price_rules
			global $wpdb;

			$pr = $this->get_num_price_rules($post_id); // cache result before deletion
			for ($i = $num_price_rules + 1; $i <= $pr; $i ++) {
				$wpdb->query($wpdb->prepare(
					"DELETE FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key LIKE %s",
					$post_id,
					"quitenicebooking_price_rule_{$i}_%"
				));
			}
		}
	}
	...
	public function template_include($template) {
		global $post;
		if (isset($post->ID) && get_post_type($post->ID) == 'accommodation' && is_single()) {
	...
	public function calc_booking_price($checkin, $checkout, $type, $entity_args) {
		...
		// get the surcharge
		$surcharge = get_post_meta($type, 'quitenicebooking_surcharge', TRUE);
		if (empty($surcharge)) {
			$surcharge = 0;
		}
		$total += $surcharge;
		return array('total' => $total, 'subtotals' => $subtotals, 'surcharge' => $surcharge);
	}

	/**
	 * Determine the lowest price of an accommodation (excluding child prices) (including price filters)
	 *
	 * @param int $type The accommodation type
	 * @return float The lowest price
	 */
	public function get_lowest_price($type) {
		$room = $this->get_single_accommodation($type);

		$prices = array();
		if ($this->settings['entity_scheme'] == 'per_person') {
			foreach ($room[$type] as $meta => $val) {
				if (preg_match('/^((?!rule).)*adult_week(?:day|end)$/', $meta)) {
					$prices[] = $val;
				}
			}
		} else {
			foreach ($room[$type] as $meta => $val) {
				if (preg_match('/room_week(?:day|end)$/', $meta)) {
					$prices[] = $val;
				}
			}
		}
	...
	/**
	 * Get the number of price rules for this post, if any
	 *
	 * @global WPDB $wpdb
	 * @param int $id The post ID
	 * @return int
	 */
	public function get_num_price_rules($id) {
		global $wpdb;

		return $wpdb->get_var($wpdb->prepare(
			"SELECT COUNT(*) FROM $wpdb->postmeta WHERE post_id = %d AND meta_key LIKE %s",
			$id,
			'quitenicebooking_price_rule_%_adult_weekday'
		));
	}
