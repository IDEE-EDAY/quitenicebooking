...
class Quitenicebooking_Utilities {
	...
	public static function send_mail($from, $to, $subject, $body, $settings) {
		...
		if (!empty($settings['enable_smtp']) && !empty($settings['smtp_host']) && !empty($settings['smtp_port'])) {
			...
			$phpmailer->SMTPAuth = !empty($settings['smtp_auth']) ? TRUE : FALSE;
	...
	/**
	 * Convert a user price string to a PHP float
	 *
	 * @param string $user_price The user price
	 * @param array $settings The global settings
	 * @return float The floated price
	 */
	public static function user_price_to_float($user_price, $settings) {
		// get the decimal separator
		$separator = preg_quote($settings['currency_decimal_separator'], '/');
		$num_decimals = $settings['currency_decimals'];
		// split the integer/decimal portions
		preg_match('/(\d+)'.$separator.'?(\d*)/', $user_price, $m);
		if (!empty($m[2])) {
			if (strlen($m[2]) > $num_decimals) {
				// trim excess decimals to avoid inconsistency between database and display data
				$m[2] = substr($m[2], 0, $num_decimals);
			}
			$float_price = $m[1].'.'.$m[2];
		} else {
			$float_price = $m[1];
		}
		return floatval($float_price);
	}

	/**
	 * Format a PHP float to a user price string
	 *
	 * @param float $float_price The floated price
	 * @param array $settings The global settings
	 * @param boolean $show_thousands Show thousands separator or not
	 * @return string The formatted price
	 */
	public static function float_to_user_price($float_price, $settings, $show_thousands = FALSE) {
		// get the separators
		if (empty($float_price)) {
			$float_price = 0;
		}
		$decimal_sep = $settings['currency_decimal_separator'];
		$thousands_sep = $show_thousands ? $settings['currency_thousands_separator'] : '';
		$num_decimals = $settings['currency_decimals'];
		return number_format($float_price, $num_decimals, $decimal_sep, $thousands_sep);
	}
