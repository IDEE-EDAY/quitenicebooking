...
<div class="booking-main-wrapper">
			...
			<?php } ?>
				...
				<ul class="room-list-wrapper clearfix">
					...
					<li class="room-item clearfix <?php echo !$highlight_found ? 'room-unavailable' : ''; ?>">
						...
						<h5><a href="<?php echo get_permalink($room['id']); ?>" target="_blank"><?php echo $room['title']; ?></a></h5>
						...
						<div class="room-list-left">
							<?php if (isset($room['_thumbnail_id'])) { ?>
								...
								<a href="<?php echo get_permalink($room['id']); ?>" target="_blank"><img src="<?php echo $thumbnail[0]; ?>" alt="" /></a>
							<?php } ?>
						<!-- END .room-list-left -->
						</div>
						...
						<div class="room-list-right">
							...
							<div class="room-price">
								<p class="price"><?php _e('From', 'quitenicebooking'); ?>: <span>
									<?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($room['lowest_price'], $this->settings, TRUE), $this->settings); ?>
									</span> / <?php _e('Night', 'quitenicebooking'); ?> </p>
								<?php if ( $highlight_found ) { ?>
									...
									<div id="price_break_hotel_room_<?php echo $room['id']; ?>" class="hide">
										...
										<div class="page-content">
											...
											<table><tbody>
												...
												<?php } // end foreach ?>
												<?php if (!empty($room['price_breakdown']['surcharge'])) { ?>
												<tr>
													<td><?php _e('Surcharge', 'quitenicebooking'); ?></td>
													<td><?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($room['price_breakdown']['surcharge'], $this->settings, TRUE), $this->settings); ?></td>
												</tr>
												<?php } ?>
												<tr>
													<td><?php _e('Room subtotal', 'quitenicebooking'); ?></td>
													<td><?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($room['price_breakdown']['total'], $this->settings, TRUE), $this->settings); ?></td>
												</tr>
											</tbody></table>
	...
	<?php if (empty($quitenicebooking_highlight) || 
			(!empty($quitenicebooking_highlight) && !(count($quitenicebooking_available_rooms) == 1 && isset($quitenicebooking_available_rooms[$quitenicebooking_highlight])) )) { ?>
	...
	<div class="booking-side clearfix">
		...
		<div class="display-reservation">
			<ul>
				...
				<li>
					<span><?php _e('Guests', 'quitenicebooking'); ?>: </span>
					<?php $str = ''; ?>
